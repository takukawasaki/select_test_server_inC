...
int
client_socket_with_timout(const char *hostnm, const char *service, int timeout)
{
     
     int sockfd;
     sockfd = tcp_connect_with_timeout(hostnm, service, timeout);
     return (sockfd);
}
...
main(int argc, char *argv[])
{
    ...
    if (argc <= 3) {
         err_sys("usage: client <host addr> <port#> timeout\n");
    }
    /* サーバにソケット接続 */
    if ((soc = client_socket_with_timout(argv[1], argv[2],atoi(argv[3]))) == -1) {
